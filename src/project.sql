...
-- start query1_opt
select sum(lo_extendedprice*lo_discount) as revenue from lineorder_opt_1, dwdate 
where lo_orderdate=d_datekey 
and d_year=1997 
and lo_discount between 1 and 3 
and lo_quantity < 24;
...
-- start query2_opt
select sum(lo_revenue), d_year, p_brand1 from lineorder_opt_2, dwdate, part, supplier
where lo_orderdate = d_datekey 
and lo_partkey = p_partkey 
and lo_suppkey = s_suppkey
and p_category = 'MFGR#12' 
...
-- start query3_opt
select c_city, s_city, d_year, sum(lo_revenue) as revenue from 
customer, lineorder_opt_3, supplier, dwdate
where lo_custkey = c_custkey 
and lo_suppkey = s_suppkey and lo_orderdate = d_datekey 
and (c_city='UNITED KI1' or c_city='UNITED KI5') 
and (s_city='UNITED KI1' or s_city='UNITED KI5') 
